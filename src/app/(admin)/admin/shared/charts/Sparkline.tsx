...
interface Props {
  ...
  strokeWidth?: number;
  /**
   * Whether to show a marker on the last point. Default: false.
   *
   * <p>Default off because under {@code preserveAspectRatio="none"} the
   * viewBox is stretched horizontally to fit the container, which warps
   * any circle into a long ellipse — the "comet tail" effect operators
   * complained about. When you do want a marker, set this to true AND
   * keep the parent container at native aspect ratio (or the marker
   * still looks stretched).
   */
...
export function Sparkline({
  ...
  return (
    ...
    >
      ...
      {/*
        vector-effect="non-scaling-stroke" tells the browser to keep the
        stroke at exactly `strokeWidth` device pixels regardless of how
        the SVG itself is scaled (preserveAspectRatio="none" stretches
        the viewBox to fit the parent, which would otherwise inflate the
        line ~15× horizontally on wide containers and look like a thick
        ribbon). With this attribute the line stays a crisp 1-1.5 px
        hairline at any container width.
      */}
      <path
        d={path}
        fill="none"
      ...
      />
      {showLastPoint && (
        // Sharper, smaller, NO glow filter — drop-shadow stretches with
        // the viewBox under preserveAspectRatio="none" and produces the
        // visible "comet tail". A flat 2-px dot reads as a marker
        // without stretching into a ribbon.
        <circle
          cx={last[0]}
          cy={last[1]}
          r={1.5}
