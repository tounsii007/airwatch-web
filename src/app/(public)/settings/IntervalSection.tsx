...
import { ChipGroup } from '@/app/(public)/settings/ChipGroup';
...
function optionsFor(language: AppLanguage) {
  return INTERVAL_SECS.map((v) => ({ value: v, label: `${MINUTE_VALUE[v]} ${t('interval_min', language)}` }));
}

/** Update-interval chip group (1 / 3 / 5 / 10 min). */
export function IntervalSection({ updateInterval, language, onChange }: Props) {
  return (
    <SectionPanel icon={<RefreshCw size={12} />} title={t('update_interval', language)}>
      <div className="py-2">
        <ChipGroup options={optionsFor(language)} value={String(updateInterval)} onChange={(v) => onChange(Number(v))} />
      </div>
    </SectionPanel>
  );
}
