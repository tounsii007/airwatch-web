 ...
 *   require a new value on every response, which only middleware can emit.
 *   Once the nonce is propagated to <NextScript>, the browser will accept
 *   ONLY inline scripts whose nonce matches — XSS via injected
 *   <script>...</script> is blocked even though we still allow the
 *   framework's own inline runtime.
 *
 * What runs:
 *   1. Generate a fresh 128-bit nonce (cryptographically random, base64).
 ...
 *
 * Performance:
 *   The runtime is the Edge runtime (V8 isolate) — middleware adds ~1 ms
 ...
 *   request and the static-config path is faster than middleware for them.
 */
...
// Skip middleware for static assets and pre-rendered output — those
// don't have inline scripts that need nonces, and adding ~1 ms per
// tile request would be wasteful.
export const config = {
  matcher: [
    /*
...
export function middleware(request: NextRequest) {
  // 128-bit cryptographically random nonce, base64-url encoded.
  // crypto.randomUUID is available in the Edge runtime; we strip dashes
  // so the value is shorter and remains URL-safe in headers.
